...
Reconstructed log of the first full execution of the
[`target-acquisition.md`](../../target-acquisition.md) funnel.  Narrative order
comes from retained artifacts and agent transcripts; a clock time is stated
only when artifact metadata supports it, and all stated clock times are UTC.
...
`build_pool.py` on snapshots (problems.yaml, an initial
vibemathed-live-20260801 snapshot,
...
The retained VibeMathed refresh is timestamped 09:30:14 UTC: 247 → 250 entries
since the prior snapshot, but distinct claimed problem numbers unchanged at
130 — **no pool member was claimed overnight**.  The pool provenance was
regenerated against that refresh at 10:50:34 UTC and records its hash.

## Tier 1 wave 1 — formalized-open slice (271 problems)
...
**Result: zero probe-grade candidates.**  Promotion requires total ≥ 8,
leverage ≥ 2, uncertainty ≥ 2, reachability ≥ 1, verification ≥ 1, a
recognition path, and no known full solution.  Best totals were 7 (one 8 with
leverage 1: #727).
...
## Tier 1 wave 2 — non-formalized-open slice (269 problems)
...
The exact private inputs and merge are immutably committed, without disclosing
their contents, by [`nonformalized-commitment.json`](nonformalized-commitment.json).

## Side check — no fresh problem additions
