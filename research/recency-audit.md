...
| #274 | the live page still marks the conjecture open and cites the all-groups order `<1440` theorem; a 2025/2026 paper proves the conjecture for simple and symmetric groups, but the searched record contains no matching verdict for the 25 named solvable non-supersolvable order-1440 groups | publish only the explicit 25-group computational exclusion, with no all-order-1440 or priority claim |
